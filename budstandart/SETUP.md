...
## GUI інтерфейс - що доступно в AI Studio?

### Базовий File Upload в AI Studio

**Доступ:** https://aistudio.google.com/

1. Відкрий AI Studio
2. Створи новий prompt або chat
3. Натисни кнопку 📎 (attachment) або drag & drop файл
4. Upload PDF/TXT документ (до 2 GB)
5. Задай питання про вміст документа

**Що це дає:**
- ✅ Швидке тестування одного документа
- ✅ Питання-відповідь про вміст файлу
- ✅ Файл зберігається 48 годин

**Що це НЕ дає (обмеження):**
- ❌ **Не** File Search stores - файл видаляється через 48 год
- ❌ **Не** можна зберігати колекції документів
- ❌ **Не** можна робити semantic search по багатьох файлах
- ❌ **Не** можна переглядати завантажені файли
- ❌ **Не** можна керувати embeddings

### File Search через GUI - НЕДОСТУПНО

⚠️ **Важливо:** Google **не надає GUI** для управління File Search stores.

File Search stores (постійні колекції документів з embeddings) доступні **тільки через API**:
- Python SDK
- JavaScript SDK
- REST API
...
# Додати багато документів
node browser-gemini-upload.js upload doc1.pdf --store krtm-regulations
node browser-gemini-upload.js upload doc2.pdf --store krtm-regulations

# Пошук по ВСІХ документах одночасно
node browser-gemini-upload.js search "вимоги до безпеки" --store krtm-regulations

# Переглянути що завантажено
node browser-gemini-upload.js list-stores
node browser-gemini-upload.js list-files --store krtm-regulations
```

**Файли зберігаються назавжди** (не 48 годин як в AI Studio)!
